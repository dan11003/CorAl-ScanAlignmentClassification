...
rosrun alignment_checker evaluate_scans --input-file-path ${BAG_FILE_PATH} --output-dir ${OUTPUT_EVAL_PATH} --eval-name ${EVAL_NAME} --sequence ${SEQUENCE} --method Coral --scan-type kstrongStructured --range-error 0.9  --rosbag-offset 0 --frame-delay 0.0 --scan-min-distance 0 --visualization --resolution 1 --radius-association 1 --input-odom-topic ${odom_topic} --compensate --entropy-config 1 ##any=0, non_zero=1, abs=2
